...
public class King extends Figure {
    ...
    public boolean isValidMove(int new_x, int new_y, Game game) {
        ...
        if (game.getChessboard()[new_y][new_x] != null && game.getChessboard()[new_y][new_x].isWhite() == isWhite()) return false;
        ...
        if (diffX == 2 && getNOfMoves() == 0) {
            if (new_x > getCoordX()) {
                // castle u desno >>>>>
                Figure rook = game.getChessboard()[getCoordY()][7];
                if (rook.getNOfMoves() == 0) {
                    if (isWalkable(new_x, new_y, game.getChessboard()) && rook.isValidMove(new_x-1, new_y, game)) {
                ...
                // castle u desno <<<<<
                Figure rook = game.getChessboard()[getCoordY()][0];
                if (rook.getNOfMoves() == 0) {
                    if (isWalkable(new_x, new_y, game.getChessboard()) && rook.isValidMove(new_x+1, new_y, game)) {
                        Move move = new Move(rook.isWhite(), rook.getCoordX(), rook.getCoordY(), new_x+1, new_y, rook);
                        rook.move(new_x+1, new_y);
        ...
        // preverja če bo ta premik postavil kralja v šah
        if (isWhite()) {
            for (Figure figure : game.getBlackFigures()) {
                if (figure.isValidMove(new_x, new_y, game) && figure != this) {
                    return false;
                }
            }
        } else {
            for (Figure figure : game.getWhiteFigures()) {
                if (figure.isValidMove(new_x, new_y, game) && figure != this) {
                    return false;
                }
            }
